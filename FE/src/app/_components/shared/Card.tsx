...
export default function Card({ video }: CardProps) {
  ...
  const getIdOfplatform = (media: Media): number => {
    if (!media || media === undefined || media === null) {
      throw new Error(`Media is empty or not an acceptable value: ${media}`)
    }
    console.log('Video object structure:', JSON.stringify(video, null, 2));    
    console.log(typeof(media.youtubeId))
    if (media.youtubeId === undefined) {
      throw new Error(`YoutubeId is null or not an acceptable value: ${media.youtubeId}`)
    }
    ...
    if (platformId === 0) {
      throw Error('Not assigned any id, platformId is 0');
    }
    return platformId;
  }
