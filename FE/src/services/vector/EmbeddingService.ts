...
export class EmbeddingService {
  ...
  async initializeEmbeddings(
    ...
    try {
      ...
      const cachedEmbeddings =
        await this.cacheService.getCachedCategoryEmbeddings();

      // if cache found then use it from centralised zustand store
      if (
        cachedEmbeddings &&
        Object.keys(cachedEmbeddings).length === Object.keys(categories).length
      ) {
        console.log(
          `Using cached embeddings for ${
            Object.keys(cachedEmbeddings).length
          } categories`
        );
  ...
  async generateBatchEmbeddings(contents: string[]): Promise<number[][]> {
    try {
      if (!contents || contents.length === 0)
        throw new Error(`The content is not valid`);
      ...
      console.log(
        `Embedding ${validContents.length} valid documents out of ${contents.length} total`
      );
      console.log(
        "First content sample:",
        validContents[0]?.substring(0, 100) + "..."
      );
      console.log(
        "Full validContents:",
      ...
      if (!Array.isArray(result)) {
        throw new Error(
          `Expected array result, got ${typeof result}: ${JSON.stringify(
            result
          )}`
        );
      }
      ...
      for (let i = 0; i < result.length; i++) {
        if (!Array.isArray(result[i])) {
          throw new Error(
            `Embedding at index ${i} is not an array: ${typeof result[i]}`
          );
        }
  ...
  async generateCategoryEmbeddings(): Promise<Record<string, number[]>> {
    ...
    try {
      console.info(
        `Generating batch embeddings for ${categoryEntries.length} categories`
      );
      ...
      const embeddings: number[][] = await this.generateBatchEmbeddings(
        descriptions
      );
      ...
      console.log(
        `Successfully generated embeddings for all ${categoryNames.length} categories`
      );
    } catch (error) {
