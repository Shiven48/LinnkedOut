import { PipelineStageResult, YoutubeMetadata, QualityMetrics, YTStatsAndTopics, YTVideoSnippet, ScoreExtractionResult } from "@/services/common/types";
...
export class YoutubeFilterService {
    ...
    public processVideos(rawVideos: YoutubeMetadata[]): ScoreExtractionResult[] {
        try {
            const uniqueVideos = this.removeDuplicateVideos(rawVideos).videos;
            
            if (uniqueVideos.length < 9) {
                throw new Error(`Critical shortage: Only ${uniqueVideos.length} unique videos, need minimum 9`);
            }
            ...
            .map((video:YoutubeMetadata) => {
                const stats:YTStatsAndTopics = this.youtubeMetadataService.extractStats(video)
                const qualityMetrics:QualityMetrics = this.extractQualityMetrics(stats, video)
                qualityMetrics.qualityScore = this.calculateQualityScore(qualityMetrics, video.snippet);
                return {
    ...
    private removeDuplicateVideos(videos: YoutubeMetadata[]):PipelineStageResult {
        ...
        const result:YoutubeMetadata[] = Array.from(uniqueVideos.values());
        ...
        return {
            ...
            filteringReason: `Removed ${videos.length - result.length} duplicate videos`
        };
    }
    ...
    private extractQualityMetrics(stats: YTStatsAndTopics, metadata: YoutubeMetadata):QualityMetrics {
        ...
        const duration = this.youtubeMetadataService.parseDurationToMs(metadata.contentDetails.duration) / 1000
    ...
    private calculateQualityScore(metrics: QualityMetrics, snippet: YTVideoSnippet): number {
        let score = 0;
        ...
        // Positive indicators
        const qualityKeywords = ['tutorial', 'guide', 'complete', 'comprehensive', 'advanced', 'professional'];
        ...
        // Negative indicators
        const clickbaitKeywords = ['shocking', 'unbelievable', 'you wont believe', 'clickbait'];
        const clickbaitMatches = clickbaitKeywords.filter(keyword => 
            title.includes(keyword) || description.includes(keyword)
        ).length;
