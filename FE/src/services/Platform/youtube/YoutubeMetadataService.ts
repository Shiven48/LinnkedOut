...
export class YoutubeMetadataSevice {
  public extractMediaData = (youtubeMetaData: any, userId: string): Media => {
    ...
    const type: "short" | "image" | "video" | "photo" =
      this.determineVideoType(durationMs);
  ...
  public extractTags = async (
    fetchedYoutubeMetadata: any,
    mediaData: Media,
    ...
    if (!tags || tags === undefined) {
      console.warn(
        "Tags are not present for the following media, generating tags..."
      );
  ...
  public generateTags = async (
    mediaData: Media,
    youtubeData: YoutubeMedia
  ): Promise<string[]> => {
    const processingService = new ProcessingService();
    const summaryService = new SummaryService();
    const preprocessedDataForTags: string =
      processingService.extractAndPreprocessData(mediaData, youtubeData);
    return await summaryService.generateTags(preprocessedDataForTags);
  };

  public extractYoutubeData = async (
    youtubeMetaData: any
  ...
  public determineVideoType = (
    durationMs: number
  ): "short" | "image" | "video" | "photo" => {
