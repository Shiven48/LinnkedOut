...
export class YoutubeTranscriptService {
  ...
  public async fetchTranscript(videoId: string, title: string): Promise<CaptionItem[]> {
    try {
      let transcriptItems: TranscriptResponse[] = await YoutubeTranscript.fetchTranscript(videoId);
  ...
  public extractEnglishCaptions(transcriptItems: TranscriptResponse[]): CaptionItem[] {
    try {
      const enTranscripts: CaptionItem[] = transcriptItems.filter((item: any): item is CaptionItem =>
        item.lang !== undefined && (item.lang === 'en' || item.lang === 'English'));
  ...
  public async fetchAndSaveAudioFromPlatform(videoId: string, title: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const process: ChildProcessWithoutNullStreams = spawn(`yt-dlp`, [
        ...
        '-o', `${path.join(`${Audio_Output_Directory}`, `${title}.${extension}`)}`,
        `${videoId}`
      ]);
      ...
      process.on('close', (code) => {
        if (code === 0) {
          resolve(path.join(`${Audio_Output_Directory}`, `${title}.${extension}`));
        } else {
