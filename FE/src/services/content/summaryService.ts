...
import { AdvancedQueryResult, CategoryClassification, EnhancedSearchQueryResult, SearchQueryResult, StreamChunk } from '@/services/common/types';
...
export class SummaryService {
  ...
  private async generateAdvancedQuery(
    ...
    try {
      ...
      return {
        ...
        alternativeQueries: [],
        qualityIndicators: this.utils.getQualityTermsForCategory(classification.categoryId),
        antiAlgorithmTerms: []
      };
    }
  }
  ...
  private selectOptimalQuery(
    ...
    const alternativeScores = queryResult.alternativeQueries.map(query => 
    ...
    if (similarity === 'high' && confidence === 'high') {
      const bestAlternative = alternativeScores.find(alt => alt.score > primaryScore);
  ...
  private optimizeQueryForQuality(query: string, categoryId: string): string {
    ...
    if (!hasQualityIndicator && this.calculateQueryQuality(optimizedQuery, categoryId) < 30) {
      const categoryQualityTerms = this.utils.getQualityTermsForCategory(categoryId);
  ...
  private generateFallbackQuery(
    ...
    const cleanTags = customTags.map(tag => tag.replace('#', '').trim());
    const qualityTerms = this.utils.getQualityTermsForCategory(classification.categoryId);
