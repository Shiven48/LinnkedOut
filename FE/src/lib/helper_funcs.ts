...
export class HelperFunctions {
  ...
  static async testFlow(data: FormDataType, userId: string): Promise<any> {
    const summaryService = new SummaryService();
    const youtubeAPIService = new YoutubeAPIService();
    const embeddingService = new EmbeddingService();
    ...
    console.log(
      `Fetched ${videoResults.length} videos from the youtube search api`
    );

    // Filtering
    const filteredVideos: ScoreExtractionResult[] =
      filterService.processVideos(videoResults);
    ...
    const extractedYTVideoData: {
      ...
      filteredVideos.map(async (scoreResult: ScoreExtractionResult) => {
        return (
          await youtubeMetadataService.parallelExtractYoutubeMedia([
            scoreResult.video,
          ])
        ).shift()!;
      })
