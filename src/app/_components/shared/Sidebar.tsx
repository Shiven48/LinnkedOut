...
export default function AppSidebar() {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`w-7 h-7 rounded-xl flex justify-center relative bg-white border-2 border-black flex-shrink-0
          ${!isOpen ? "right-3" : ""}`}
        >
          <Image
            src="/home.svg"
      ...
      <nav className="mt-6 space-y-2 bg-inherit">
        {categories().map((category: Category) => (
          <div
            key={category.title}
            className={`rounded-xl w-full hover-side transition ease-out 0.3s shadow shadow-white
          ...
          >
            <Link
              ...
              className={`bg-golden flex items-center py-1 px-4 rounded-xl transition-all duration-200 ease-in-out relative hover-side text-black shadow border border-black
                ${
                  activeCategory === category.title
                    ? "bg-[#e3ec58] shadow shadow-[#e3ec58]"
                    : "shadow-white"
                }`}
            >
              <div
